 ...
 *
 * Passamos como argumentos da funcao o estado, o char l e o char c que sao as posicoes jogadas pelo utilizador.
 * Convertemos esses chars em inteiros e, com 2 ciclos percorremos o tabuleiro até à posicao que foi jogada, aí,
 * nessa posicao, percorremos as 8 posicoes à sua volta e verificamos se em alguma dessas posicoes existe alguma peça
 * adversario. Caso existam, percerremos sempre essa direção enquanto temos peças adversarias.Quando uma posicao dessa
 * direcao ja nao é uma peca adversaria mas sim a peca que está a ser jogada,entao, aí, voltamos a percorrer tudo para
 * trás até à posição jogada e enquanto percorremos vamos alterando as peças.
 *
 */
...
*/
ESTADO jogar_peca (ESTADO e, char *l,char *c, char *flag) {
  ...
  *
  * FUNCIONAMENTE:
  *     Basicamente, para colocar os pontos fomos alterando o estado do jogo. Com 2 ciclos for percorremos o tabuleiro
  *     e sempre que encontramos uma peça igual à que está a ser jogada percorremos todas as posições à sua volta.
  *     Verificamos se essas posições sao uma jogada possivel e se alguma dela é contem uma peça adversaria. Caso alguma
  *     dessa posições tenha uma peça adversária vamos percorrendo nessa direção até chegarmos a uma posição vazia.
  *     Nessa posição vazia colocámos um ponto.
