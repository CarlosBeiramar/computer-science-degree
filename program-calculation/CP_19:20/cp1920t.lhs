...
discollect x = set[ k |-> y | (k,d) <- x , y <- d]
...
tar = cataExp g where
  g = either g1 g2 where
    g1 v = [([],v)]
    g2 (o,listapares) = map((o++) >< id) $ concat listapares

dic_rd s d = lookup (map toUpper s) (dic_exp d)
...
isOrd' = cataBTree g
  where g = undefined
isOrd Empty = True
isOrd (Node(r,(Empty,Empty))) = True
isOrd (Node(r,(Node(r1,(esq1,dir1)),Empty))) |r >= r1 = isOrd esq1 && isOrd dir1
                                               |otherwise = False
isOrd (Node(r,(Empty,(Node(r1,(esq1,dir1)))))) |r < r1 = isOrd esq1 && isOrd dir1
                                               |otherwise = False
isOrd (Node(r,(Node(r1,(esq1,dir1)),(Node(r2,(esq2,dir2)))))) |r >= r1 && r < r2 = isOrd (Node(r1,(esq1,dir1))) && isOrd (Node(r2,(esq2,dir2)))
                                                                |otherwise = False
...
splay = flip(cataBTree (either g1 g2)) where
    g1 l a = Empty
    g2 (a,(esq,dir)) [] = Node(a,(esq [],dir []))
    g2 (a,(esq,dir)) (h:t) | h == True = esq t
                           | otherwise = dir t
...
\begin{eqnarray}
\xymatrix@C=4cm{
    \ensuremath{\Varid Bdt A}
&
    \ensuremath{\Varid Dec \mathbin{+}\Varid String\times (\Varid Bdt A \times Bdt A)\ar[l]_-{\ensuremath{\mathsf{in}}}}
\\
     \ensuremath{\mathit B}\ar[u]^-{\ensuremath{\ana{\Varid{g}}}}\ar[r]^-{\ensuremath{\Varid{g}}}
&
     \ensuremath{\Varid{Dec}\mathbin{+}\Varid String\times (\Varid B \times\Varid B)}\ar[u]_{\ensuremath{\Varid{id}\mathbin{+}\mathbin{(}\Varid{id}\times\mathbin{(}\Varid{g}\times\Varid{g}\mathbin{)}\mathbin{)}}}
}
\end{eqnarray}
...
bnavLTree = cataLTree (either g1 g2) where
    g1 a _ = Leaf a
    g2 (l,r) Empty = Fork (l Empty, r Empty)
    g2 (l,r) (Node (bool,(esq,dir))) |bool == True = l esq
                                     |otherwise = r dir
...
main :: IO()
main = do m <- matrix (10,10) 
          display janela white $ Pictures $ mosaicos m (inicioX,inicioY)

mosaicos :: [[Bool]] -> (Float,Float) -> [Picture]
mosaicos [] _ = []
mosaicos ([]:t) (x,y) = mosaicos t (inicioX , y - tamanho)
mosaicos ((h:t):s) (x,y) | not h = (put (x,y) truchet1) : mosaicos (t:s) (x + tamanho , y)
                         | otherwise = (put (x,y) truchet2) : mosaicos (t:s) (x + tamanho , y)

matrix (x,y) = sequence $ replicate x $ sequence $ replicate y $ randomRIO (True,False::Bool)
...
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=0.90\textwidth]{Mosaico.png}\par
            \caption{Um mosaico de Truchet-Smith.}
            \label{img:model diagram}
    \end{center}
\end{figure}
...
%----------------- Fim do documento -------------------------------------------%
\end{document}
d=[("ABA",["BRIM"]),("ABALO",["SHOCK"]),("AMIGO",["FRIEND"]),("AMOR",["LOVE"]),("MEDO",["FEAR"]),("MUDO",["DUMB","MUTE"]),("PE",["FOOT"]),("PEDRA",["STONE"]),("POBRE",["POOR"]),("PODRE",["ROTTEN"])]